...
async def seed():
    ...
    async with AsyncSessionLocal() as db:
        # Admin
        r = await db.execute(select(User).where(User.email == "admin@casafernando.com"))
        ...
        # Mesonera
        r = await db.execute(select(User).where(User.email == "mesonera@casafernando.com"))
        if not r.scalar_one_or_none():
            mesonera = User(
        ...
        # Punto de venta
        r = await db.execute(select(User).where(User.email == "pos@casafernando.com"))
        if not r.scalar_one_or_none():
            pos_user = User(
        ...
        for nombre, desc, orden in categorias_data:
            r = await db.execute(select(CategoriaPlato).where(CategoriaPlato.nombre == nombre))
            if not r.scalar_one_or_none():
        ...
        for nombre, desc, precio, cat in platos_data:
            r = await db.execute(select(Plato).where(Plato.nombre == nombre))
            if not r.scalar_one_or_none() and cat in cat_ids:
                p = Plato(
        ...
        # Mesas
        for i in range(1, 11):
            r = await db.execute(select(Mesa).where(Mesa.numero == str(i)))
            if not r.scalar_one_or_none():
